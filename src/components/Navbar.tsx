...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="no-print sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-slate-200/90 shadow-2xs w-full max-w-full transition-all">
      <div className="w-full max-w-7xl mx-auto px-2.5 sm:px-6 lg:px-8 box-border min-w-0 overscroll-x-contain">
        
        {/* ========================================================= */}
        {/* BARIS 1: IDENTITAS APLIKASI, PROFIL SEKOLAH & AKSI GLOBAL */}
        {/* ========================================================= */}
        <div className="w-full max-w-full overflow-x-auto overflow-y-visible scrollbar-none touch-pan-x overscroll-x-contain">
          <div className="flex items-center justify-between h-13 sm:h-14 gap-2 sm:gap-4 min-w-max sm:min-w-0 sm:w-full">
            
            {/* Kiri: Brand & Sinkronisasi */}
            <div className="flex items-center gap-2 sm:gap-3 shrink-0">
              ...
              >
                <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-gradient-to-br from-emerald-600 to-emerald-800 flex items-center justify-center text-white font-extrabold text-xs sm:text-sm shadow-sm ring-2 ring-emerald-500/20 group-hover:scale-105 transition-transform shrink-0">
                  SR
                </div>
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs sm:text-sm font-bold tracking-tight text-slate-900 leading-tight truncate">
                      SIAP-Revita
                    </span>
                    <span className="text-[9px] sm:text-[10px] bg-emerald-100 text-emerald-800 font-bold px-1.5 py-0.2 rounded-full border border-emerald-200">
                      2026
                    </span>
                  </div>
                  <span className="text-[9px] text-slate-500 block leading-none font-medium truncate">
              ...
              {/* Status Cloud Sync */}
              <div
                className="hidden md:flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-medium border shrink-0 bg-slate-50 text-slate-600 border-slate-200 shadow-2xs"
                title={
              ...
              >
                {cloudSyncStatus === 'SYNCING' ? (
                  <>
                    ...
                    <span className="text-sky-600">Menyimpan...</span>
                  </>
                ) : cloudSyncStatus === 'ONLINE_SYNCED' ? (
                  <>
            ...
            {/* Kanan: Profil Sekolah Aktif & Aksi Berkas */}
            <div className="flex items-center gap-1 sm:gap-2 shrink-0">
              ...
              <div className="relative min-w-0" ref={dropdownRef}>
                <button
                  ...
                  className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-2.5 py-1 bg-slate-50 hover:bg-emerald-50 border border-slate-200 hover:border-emerald-300 text-slate-800 rounded-xl transition-all shadow-2xs text-xs font-medium max-w-[150px] sm:max-w-[220px] md:max-w-[270px] min-w-0"
                  title="Ganti Profil / Sekolah Aktif"
                >
                  <div className="w-5 h-5 rounded-lg bg-emerald-700 text-white flex items-center justify-center font-bold text-[9px] shrink-0 shadow-2xs">
                  ...
                  <div className="text-left truncate min-w-0">
                    ...
                    <div className="flex items-center gap-1 text-[9px] text-emerald-800 font-mono truncate">
              ...
              <button
                ...
                className="relative hidden sm:flex items-center gap-1.5 px-2.5 py-1 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-semibold transition-all shadow-2xs shrink-0 active:scale-95"
              ...
              <button
                ...
                className="hidden md:flex items-center gap-1.5 px-2.5 py-1 bg-white hover:bg-slate-50 border border-slate-300 text-slate-700 rounded-xl text-xs font-semibold transition-colors shadow-2xs shrink-0"
              ...
              <button
                onClick={onPrintLPJ}
                className="flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold text-white bg-slate-800 hover:bg-slate-900 rounded-xl transition-colors shadow-xs shrink-0"
              ...
              <button
                ...
                className="p-1.5 text-slate-500 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition-colors shrink-0"
        ...
        {/* ========================================================= */}
        {/* BARIS 2: NAVIGASI MODUL LENGKAP (TABS UTAMA)              */}
        {/* ========================================================= */}
        <div className="w-full max-w-full overflow-x-auto py-1.5 border-t border-slate-100 scrollbar-none touch-pan-x overscroll-x-contain select-none">
          <nav className="flex items-center gap-1 min-w-max px-0.5">
            {navLinks.map((link) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-semibold rounded-xl transition-all whitespace-nowrap active:scale-95 shrink-0 ${
                    isActive
                      ? 'bg-slate-900 text-white font-bold shadow-xs'
                      : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100/90 bg-slate-50/50'
                  }`}
                >
                    ...
                    <span
                      className={`text-[9px] px-1.5 py-0.2 rounded-full font-medium ${
                        isActive
                          ...
                          : link.badgeColor || 'bg-slate-200 text-slate-700'
                      }`}
                    >
                      {link.badge}
                    </span>
        ...
        {/* ========================================================= */}
        {/* BARIS 3: INDIKATOR METRIK REAL-TIME & MENU PINTAS AKSI    */}
        {/* ========================================================= */}
        <div className="w-full max-w-full overflow-x-auto py-1.5 border-t border-slate-100 scrollbar-none touch-pan-x overscroll-x-contain">
          <div className="flex items-center justify-between gap-2 text-xs min-w-max">
            
            {/* Sisi Kiri: Live Finansial & Progres Real-time */}
            <div className="flex items-center gap-2 shrink-0">
              ...
              >
                ...
                <div className="flex items-center gap-1 text-[10px] text-slate-500 border-l border-slate-200 pl-1.5">
              ...
              <div
                onClick={() => setActiveTab('overview')}
                className="cursor-pointer flex items-center gap-1.5 bg-slate-50 hover:bg-slate-100 text-slate-800 px-2.5 py-1 rounded-lg border border-slate-200/80 transition-colors shadow-2xs shrink-0"
                title="Serapan Anggaran vs Total Pagu Dana"
              >
                <span className="text-[11px] font-medium text-slate-500">Serapan:</span>
                <div className="w-14 sm:w-18 bg-slate-200 h-2 rounded-full overflow-hidden shrink-0">
                  <div
              ...
              >
                ...
                {progressPhotos.length > 0 && (
                  <span className="inline-flex items-center gap-0.5 text-[10px] bg-teal-100 text-teal-800 px-1 rounded font-medium">
              ...
              {/* Status Integritas Kas */}
              {summary.isBalanceSynced ? (
                <span className="inline-flex items-center gap-1 text-[10px] text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-md font-semibold shrink-0">
                  <CheckCircle2 className="w-3 h-3 text-emerald-600" />
                  Kas Seimbang
                </span>
              ) : (
                <span className="inline-flex items-center gap-1 text-[10px] text-amber-800 bg-amber-50 border border-amber-300 px-2 py-0.5 rounded-md font-bold animate-pulse shrink-0">
            ...
            {/* Sisi Kanan: Menu Pintas & Aksi Cepat Finansial */}
            <div className="flex items-center gap-1.5 shrink-0">
              
              {/* Tombol Cepat: Catat Transaksi Kas */}
              <button
                onClick={onOpenAddTx}
                className="flex items-center gap-1 px-2.5 py-1 bg-emerald-700 hover:bg-emerald-800 text-white rounded-lg text-xs font-semibold transition-colors shadow-2xs shrink-0 active:scale-95"
                title="Catat Transaksi Penerimaan atau Pengeluaran Kas BKU"
              >
                <Plus className="w-3 h-3" />
                <span>+ Catat Kas</span>
              </button>

              {/* Tombol Cepat: Tarik Kas Bank-Tunai */}
              <button
                onClick={onOpenTransfer}
                className="flex items-center gap-1 px-2.5 py-1 bg-sky-700 hover:bg-sky-800 text-white rounded-lg text-xs font-semibold transition-colors shadow-2xs shrink-0 active:scale-95"
                title="Pindah Buku / Tarik Tunai Rekening Bank ke Kas Tunai"
              >
                <ArrowRightLeft className="w-3 h-3" />
                <span>+ Tarik Kas</span>
              </button>

              {/* Dropdown Menu Pintas Aksi Lengkap */}
              <div className="relative" ref={quickActionsRef}>
                <button
                  onClick={() => setIsQuickActionsOpen(!isQuickActionsOpen)}
                  className="flex items-center gap-1 px-2.5 py-1 bg-slate-800 hover:bg-slate-900 text-white rounded-lg text-xs font-bold transition-all shadow-2xs active:scale-95 shrink-0"
                  title="Aksi Pintas Lainnya"
                >
                  <Zap className="w-3 h-3 text-amber-300 animate-pulse shrink-0" />
                ...
                {isQuickActionsOpen && (
                  <div className="absolute right-0 mt-2 w-72 sm:w-80 bg-white rounded-2xl shadow-2xl border border-slate-200 py-2 z-50 animate-in fade-in zoom-in-95">
                    ...
                    <div className="p-1.5 space-y-1">
                      ...
                      >
                        ...
                        <div>
                          <span className="text-xs font-bold text-slate-800 block group-hover:text-indigo-900">
                            Input SPJ Upah Tukang
                          </span>
                          <span className="text-[10px] text-slate-500 block">
                            Hitung lembur, kasbon, & tanda tangan mingguan
                          </span>
                        </div>
                      </button>
                      ...
                      >
                        <div className="w-7 h-7 rounded-lg bg-teal-100 text-teal-700 flex items-center justify-center font-bold text-xs shrink-0">
                          <Camera className="w-3.5 h-3.5" />
                        </div>
                        <div>
                          <span className="text-xs font-bold text-slate-800 block group-hover:text-teal-900">
                            Upload Foto Progres Fisik
                          </span>
                          <span className="text-[10px] text-slate-500 block">
                            Multi-upload foto dokumentasi untuk lampiran SPJ & LPJ
                          </span>
                        </div>
                      </button>
                    </div>
